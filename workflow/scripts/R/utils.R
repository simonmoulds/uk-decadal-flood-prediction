...
parse_config_observed <- function(config, input_data_root) {
  ...
  giss <-
    file.path(input_data_root, items$giss$subdirectory) %>%
    check_file_exists()
  gpcc <-
    file.path(input_data_root, items$gpcc$subdirectory) %>%
    check_file_exists()
  hadcrut4 <-
    file.path(input_data_root, items$hadcrut4$subdirectory) %>%
    check_file_exists()
  hadslp2r <-
    file.path(input_data_root, items$hadslp2r$subdirectory) %>%
    check_file_exists()
  ncdc <-
    file.path(input_data_root, items$ncdc$subdirectory) %>%
    check_file_exists()
  list(
    giss = giss, gpcc = gpcc, hadcrut4 = hadcrut4,
    hadslp2r = hadslp2r, ncdc = ncdc
  )
}
...
parse_config_aggregation_period <- function(config) {
  ...
  for (i in 1:length(nms)) {
    ...
    error_season <- ifelse(is.null(item$error_season), NA, item$error_season)
    ...
    if (is.null(lead_time))
      stop("`lead_time` cannot be missing")

    if (is.null(study_period))
      stop("`study_period` cannot be missing")
...
get_hindcast_data <- function(dataset,
  ...
  ## Restrict to complete seasons
  group_vars <- c(
    "season_year", "project", "mip", "source_id", "member", "init_year"
  )
  ens_fcst <- ens_fcst %>%
...
download_nrfa_data <- function(stn_id) {
  ## TODO tidy up this function, giving user more
  ## control over which variables are derived
...
calculate_error <- function(fcst,
                            ensemble_fcst,
                            n_years,
                            n_forecast,
                            match_var) {
  ## Calculate error between ensemble members and ensemble mean
  ##
...
create_ensemble_forecast <- function(ensemble_fcst_error,
  ...
  ensemble_fcst =
    ...
    left_join(
      ensemble_fcst,
      by = c("project", "mip", "source_id", "member", "init_year_matched", "init_year")
    )
  ...
  ensemble_fcst =
    ...
    summarize(across(any_of(vars), mean, na.rm = TRUE))
  ensemble_fcst
}
...
load_model_predictions <- function(config, experiment, aggregation_period, season) {
  predictions = open_dataset(
    ## file.path(outputroot, "analysis", experiment, aggregation_period, "gamlss", "prediction"),
    file.path("results/output", experiment, "gamlss", aggregation_period, season, "prediction"),
...
load_model_simulations <- function(config, experiment, aggregation_period, season) {
  simulations <- open_dataset(
    file.path("results/output", experiment, "gamlss", aggregation_period, season, "simulation")
  ) %>%
    collect() %>%
    filter(subset %in% c("full", "best_n")) #%>%
    ## mutate(subset = ifelse(subset == "best_n", "NAO-matched ensemble", "Full ensemble"))
...
load_model_fit <- function(config, experiment, aggregation_period, season) {
  fit <- open_dataset(
    ...
    ## file.path(outputroot, "analysis", experiment, "gamlss", aggregation_period, "fit"),
    file.path("results/output", experiment, "gamlss", aggregation_period, season, "fit"),
    partitioning = c("ID", "model", "subset")
  ) %>% collect()
...
load_skill_scores <- function(config, experiment, aggregation_period, season) {
  ds <- open_dataset(
    ## file.path(outputroot, "analysis", experiment, "gamlss", aggregation_period, "prediction")
    ## file.path(outputroot, "analysis", experiment, aggregation_period, "gamlss", "prediction"),
    ## file.path(outputroot, "analysis", experiment, "gamlss", aggregation_period, "prediction"),
  ...
  obs_col <- grep("^Q.*_obs$", names(ds), value=TRUE)
  exp_col <- grep("^Q.*_exp$", names(ds), value=TRUE)
  if (length(obs_col) == 0 | length(exp_col) == 0)
    stop("Could not find obs|exp column!")
  ds <- ds %>% rename(obs=!!obs_col, exp=!!exp_col)
  skill <- ds %>%
    ...
    summarize(
      ...
      aic=mean(aic),
      r=cor.test(obs, exp, method="pearson")$estimate
    ) %>%
...
get_antecedent_season <- function(season_str, n_months = 3) {
  months <- "JFMAMJJASOND"
  months <- paste0(months, months, months)
  if (!str_detect(months, season)) {
    stop("Invalid season")
  }
  months_index <- str_locate(months, season_str)
  ante_end <- months_index[1] - 1
  ante_start <- ante_end - n_months + 1
  ante_index <- ante_start:ante_end
  ## Adding 12 (i.e. n months in a year) ensures the index is positive
  ante_index <- ante_index + 12
  ante_season <- paste0(strsplit(months, "")[[1]][ante_index], collapse="")
  ante_season
}

## get_observed_discharge <- function(root, id, source) {
##   if (source == "GRDC") {
##     filepath = file.path(root, 'grdc-discharge-summaries', id)
##   } else if (source == "UKBN") {
##     filepath = file.path(outputroot, 'nrfa-discharge-summaries', id)
##   }
